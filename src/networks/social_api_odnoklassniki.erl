...
validate_auth({UserID, UserData, Signature}) ->
    ...
    Data = social_api_utils:concat([UserID, UserData, SecretKey]),
...
invoke_method({Group, Function}, Args, State) ->
    ...
    SecretKey     = social_api_settings:secret_key(),
    Host          = social_api_settings:client_host("api.odnoklassniki.ru"),
...
do_send(Message, UserID, State) ->
    ...
    Args = [{uid, social_api_utils:to_list(UserID)}, {text, Message}],
    {Result, NewState} = invoke_method(Method, Args, State),
    {parse_response(UserID, Result), NewState}.

parse_response(UserID, true) -> [{social_api_utils:to_integer(UserID), ok}];
parse_response(UserID, {struct,ErrorInfo}) ->
    ...
    [{social_api_utils:to_integer(UserID), {error, {Code, ErrMsg}}}].
